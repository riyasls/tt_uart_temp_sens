...
## How to test
The input PWM signal is provided on the input pin ui_in[0], and the UART TX output is connected to the output pin uo_out[1].The design operates with a clock frequency of 50 MHz. The values that shown on the serial terminal is in the Hexadecimal form. The low time period = ( 1/clock frequency ) * (counter value shown in the serial terminal in decimal).
...
Stop Bits: 1,
Flow Control: None,
The design operates with a clock frequency of 50 MHz.
